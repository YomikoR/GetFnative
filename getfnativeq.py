...
def gen_descale_error(clip: vs.VideoNode,
    ...
    diff = core.std.Expr([clips, rescaled], f'x y - abs dup 0.015 > swap 0 ?').std.Crop(10, 10, 10, 10).std.PlaneStats()
...
def main() -> None:
    ...
    if not ext.lower() in {'.py', '.pyw', '.vpy'}:
        raise ValueError('Input must be a vpy script whose output index 0 is accepted.')
    ...
    gen_descale_error(clip, args.frame_no, base_height, base_width, src_heights, args.mode, save_path)
